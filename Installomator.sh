...
remotedesktopmanagerenterprise)
    ...
    type="dmg"
    downloadURL=$(curl -fsL "https://devolutions.net/remote-desktop-manager/download/thank-you/?platform=RDMMacbin&edition=enterprise&os=macos" | grep "const productInfo" | grep -oe "{.*}" | jq -r '."RDMMacbin.Url"')
...
snagit2026)
    name="Snagit"
    type="dmg"
    sparkleData=$(curl -fsL -H 'User-Agent: Snagit/2026.0.0' 'https://www.techsmith.com/redirect.asp?target=sparkleappcast&product=snagitmac&ver=2026.0.0&lang=enu&os=mac')
    appNewVersion=$(
        echo "$sparkleData" | \
